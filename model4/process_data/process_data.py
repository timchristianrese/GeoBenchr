...
print("Arguments reçus :", sys.argv[1:], flush=True)

def parse_point(w):
    try:
        p = wkt_loads(w);  return p.y, p.x
    except Exception:
        return None, None
...
if len(sys.argv) < 2:
    sys.exit("Usage : python process_flights_csv.py <csv1> [csv2] ...")
...
for csvfile in sys.argv[1:]:
    ...
    reader = pd.read_csv(
        ...
        sep=";",
        header=None,
        names=["track_id", "ac_type", "origin", "dest",
               "point", "ts", "alt_ft"],
        dtype={"track_id": str, "point": str, "ts": str, "alt_ft": float},
        chunksize=CHUNKSIZE
    )

    for chunk_idx, chunk in enumerate(tqdm(reader, desc="Lecture", unit="chunk")):
        ...
        tqdm.write(f"chunk {chunk_idx:>4} : {len(chunk):,} points (cumul {total_points:,})")

    for track, rows in tqdm(flights.items(), desc="Écriture vols", unit="vol"):
        write_flight(track, rows)

    print(f"Finished for {csvfile} : {len(flights):,} flights, {total_points:,} points\n")
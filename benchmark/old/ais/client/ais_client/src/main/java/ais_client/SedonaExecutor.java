...
public class SedonaExecutor implements QueryExecutor {
    ...
    public SedonaExecutor(String masterUrl) {
        ...
        // Activate Sedona SQL extensions
            ...
            String resourcePath = System.getenv().getOrDefault("RESOURCE_PATH", "/home/tim/data/ais/resources/");
            String dataPath =     System.getenv().getOrDefault("DATA_PATH", "/home/tim/data/ais/");
            ...
            geoDf.createOrReplaceTempView("crossing_points");
            ...
            Dataset<Row> harborsDf = sedona.read().format("csv").option("delimiter", ";").option("header", "true").load(resourcePath+"harbors.csv");
            harborsDf.createOrReplaceTempView("harbors");
            //convert column 1 (latitude) and column 2 (longitude) to a geometry column
            harborsDf = sedona.sql("SELECT name, ST_GeomFromWKT(CONCAT('POINT(', _c2, ' ', _c1, ')')) AS geom FROM harbors;");
            harborsDf.createOrReplaceTempView("harbors");
            harborsDf.cache().count();
            harborsDf.printSchema();
    ...
    public void execute(String sql, String queryName) {
        ...
        try {
            ...
            QueryLogger.logSuccess("Sedona", queryName, runID, duration);
        } catch (Exception e) {
            QueryLogger.logError("Sedona", queryName, runID, e);
        }
    }
}
